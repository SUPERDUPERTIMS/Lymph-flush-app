...
def scroll_to_top():
    ...
    plus removes Streamlit header bars."""
    components.html(
        """
        <script>
            setTimeout(function() {
...
<b>🎯 Goal:</b> {step_info['goal']}
        ...
        if not st.session_state.timer_running:
            ...
            def render_timer(step_info=step_info, total_time=total_duration_secs):
                ...
                if (elapsed % 10) < 5:
                    st.markdown(
                        '<div class="breath-box">🌬️ Deep Belly Inhale...</div>', unsafe_allow_html=True
                    )
                else:
                    st.markdown(
                        '<div class="breath-box">😌 Slow Relaxed Exhale...</div>', unsafe_allow_html=True
                    )
